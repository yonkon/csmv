...
class App
{
    ...
    public function init($params = array())
    {
        ...
        $base_path = isset($params['base_path']) ? $params['base_path'] : './..';
    ...
    {
        $dispatch = is_array($dispatch) ? key($dispatch) : $dispatch;
    ...
    {
        return '[' . date('r') . '] (' . $type . ')' . $title . '; ' . $message . "\r\n";
    }
    ...
    /**
     * Returns product name like "CS-Cart", "Multi-Vendor"
     *
     * @return string Formatted product name (E.g.: CS-Cart)
     */
    public function getProductName()
    {
        if (PRODUCT_EDITION == 'MULTIVENDOR') {
            $product_name = 'Multi-Vendor';
        } else {
