        ...
        <div class="ty-feature-value">
            ...
            {elseif $feature.feature_type == "M" && $feature.variants}
                ...
                {foreach from=$feature.variants item="var"}
                    ...
                    {if $var.selected}<li><span class="ty-compare-checkbox" title="{$var.variant}"><i class="ty-icon-ok"></i></span>{if !$hide_variant_affix}{$feature.prefix}{/if}{$var.variant}{if !$hide_variant_affix}{$feature.suffix}{/if}</li>{/if}
                {/foreach}
                </ul>
