    ...
    <div class="ty-features-all__group ty-column6">
            {if $ranges}
                ...
                {foreach from=$ranges item="range"}
                    ...
                    <li class="ty-features-all__list-item"><a href="{if $range.feature_type == "E"}{"product_features.view?variant_id=`$range.range_id``$cur_features_hash`"|fn_url}{else}{"`$filter_qstring`&features_hash=`$_features_hash`"|fn_url}{/if}" class="ty-features-all__list-a">{$range.range_name|fn_text_placeholders}</a></li>
                {/foreach}
            </ul>
            {else}&nbsp;{/if}
    </div>
