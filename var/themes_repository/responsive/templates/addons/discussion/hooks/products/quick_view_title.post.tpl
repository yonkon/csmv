{if $product.discussion_type && $product.discussion_type != 'D'}
    <div class="ty-discussion__rating-wrapper clearfix" id="average_rating_product_{$obj_prefix}{$obj_id}">
        ...
        <a  href="{"products.view?product_id=`$product.product_id`&selected_section=discussion#discussion"|fn_url}" class="ty-discussion__review-a">{$product.discussion.posts|count} {__("reviews", [$product.discussion.posts|count])}</a>
        {/if}
