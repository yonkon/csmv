...
{hook name="products:buttons_block"}
    ...
    {elseif ($settings.General.inventory_tracking == "Y" && $settings.General.allow_negative_amount != "Y" && (($product_amount <= 0 || $product_amount < $product.min_qty) && $product.tracking != "D") && $product.is_edp != "Y")}
        ...
        {elseif (($product.out_of_stock_actions == "S") && ($product.tracking != "O"))}
            ...
            <div class="ty-control-group ty-input-append ty-product-notify-email" id="product_notify_email" style="{if $product_notification_enabled != "Y"} display: none;{/if}">
                <form action="{""|fn_url}" method="post" enctype="multipart/form-data" class="cm-disable-empty-files cm-ajax">
                    ...
                    <input type="text" name="email" id="product_notify_email_{$obj_prefix}{$obj_id}" size="20" 
                        ...
                        class="ty-product-notify-email__input cm-hint" disabled="disabled" title="{__("enter_email")}" />
                    <button class="ty-btn-go" title="{__("go")}" alt="{__("go")}"><i class="ty-btn-go__icon ty-icon-right-dir"></i></button>
    ...
    {if $show_list_buttons}
        ...
        {if $smarty.capture.$capture_buy_now|trim}
            {if $separate_buttons}<div class="ty-add-buttons-wrap {if $cart_button_exists || (($product.out_of_stock_actions == "S") && ($product.tracking != "O"))} no-margin{/if}"><div class="add-buttons-inner-wrap">{/if}
        ...
        {if $show_qty && $product.is_edp !== "Y" && $cart_button_exists == true && ($settings.General.allow_anonymous_shopping == "allow_shopping" || $auth.user_id) && $product.avail_since <= $smarty.const.TIME || ($product.avail_since > $smarty.const.TIME && $product.out_of_stock_actions == "B")}
            <div class="ty-qty clearfix{if $settings.Appearance.quantity_changer == "Y"} changer{/if}" id="qty_{$obj_prefix}{$obj_id}">
                ...
                <div class="ty-center ty-value-changer cm-value-changer">
                    ...
                    {/if}
                    <input type="text" size="5" class="ty-value-changer__input cm-amount" id="qty_count_{$obj_prefix}{$obj_id}" name="product_data[{$obj_id}][amount]" value="{$default_amount}"{if $product.qty_step > 1} data-ca-step="{$product.qty_step}"{/if} />
                    {if $settings.Appearance.quantity_changer == "Y"}
