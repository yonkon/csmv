    ...
    public function setArea($area, $area_type = '', $company_id = null)
    {
        ...
        if (!is_dir($compile_dir)) {
            if (fn_mkdir($compile_dir) == false) {
                throw new PermissionsException("Can't create compiled templates directory: <b>" . $compile_dir . '</b>.<br>Please check if it exists, and has writable permissions.');
            }
        }

        if (!is_dir($cache_dir)) {
            if (fn_mkdir($cache_dir) == false) {
                throw new PermissionsException("Can't create template cache directory: <b>" . $cache_dir . '</b>.<br>Please check if it exists, and has writable permissions.');
            }
        }
