...
function fn_seo_validate_object($seo, $path, $objects)
{
    ...
    if (
        (in_array($seo['type'], array('p', 'a')) && empty($objects['extension'])) ||
        ($seo['type'] == 'c' && Registry::get('addons.seo.seo_category_type') != 'file' && !empty($objects['extension'])) ||
        ($seo['type'] == 'e' && empty($objects['extension']))
        ) {
        $result = false;
    }
