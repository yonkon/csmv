...
class TwigmoUpgrade
{
    ...
    final public static function getUpgradeDirs($install_src_dir)
    {
        ...
        $repo_path = 'var/themes_repository/' . TwigmoSettings::get('base_theme') . '/';
        ...
        foreach ($file_areas as $key => $file_area) {
            ...
            $dirs['repo'][$key . '_frontend'] = fn_get_theme_path(
                '[repo]/' . TwigmoSettings::get('base_theme') . '/',
                'C'
            ) . $file_area . '/' . $full_addon_path;
        }
    ...
    final public static function checkForUpgrade($display_service_notifications = true)
    {
        $is_upgradable = false;
