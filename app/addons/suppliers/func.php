...
function fn_get_supplier_shippings($supplier_id)
{
    if (!empty($supplier_id)) {
        ...
        // Ungroupped shippings
        $shippings = db_get_fields('SELECT shipping_id FROM ?:shippings WHERE shipping_id NOT IN (SELECT object_id FROM ?:supplier_links WHERE object_type = ?s)', 'S');
    }
