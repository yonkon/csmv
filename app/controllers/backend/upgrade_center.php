...
fn_define('DIR_REPOSITORY', 'var/themes_repository/' . Registry::get('config.base_theme') . '/');
 ...
 */
    ...
    if (empty($xml)) {
        ...
        fn_uc_run_continuous_job('fn_uc_create_themes', array(0, $package_path, $_SESSION['uc_package'], $skip_files, $custom_theme_files, $base_ult_theme_files));

        if (fn_allowed_for('ULTIMATE')) {
            $company_ids = fn_get_all_companies_ids();
            foreach ($company_ids as $company_id) {
                fn_uc_run_continuous_job('fn_uc_create_themes', array($company_id, $package_path, $_SESSION['uc_package'], $skip_files, $custom_theme_files, $base_ult_theme_files));
            }
        }
...
function fn_uc_create_themes($company_id, $path, $package, $skip_files, $custom_theme_files, $base_ult_theme_files, $log)
{
    ...
    if (is_file($path)) {
        ...
        if (strpos($path, DIR_REPOSITORY) !== false) {
            ...
            if (fn_allowed_for('MULTIVENDOR') || fn_allowed_for('ULTIMATE') && ($company_id || !$company_id && (strpos($path, DIR_REPOSITORY . 'mail/') !== false))) {
                foreach ($installed_themes[$company_id] as $s) {
                    ...
                    if (!fn_uc_check_array_value($path, $custom_theme_files) || $s = Registry::get('config.base_theme')) { // copy non-custom files only
        ...
        foreach ($files as $file) {
            ...
            if (!in_array($fname, $skip_files) && !(file_exists($file) && strpos($path, '/' . Registry::get('config.base_theme') . '/') !== false)) {
