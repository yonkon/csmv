(function(_, $) {
    ...
    var file_editor = {
        ...
        init: function () {
            ...
            $.ceAjax('request', fn_url('file_editor.init_view?dir=' + path), {
                cache: false,
                callback: function(data) {
                    var file_count = $($.parseHTML(data.files_list)).find('li').length;
                    if(file_count == 0) {
                        data.files_list = '<ul><li class="muted center">' + _.tr('text_no_files_uploaded_yet') + '</li></ul>';
                    }
                    initFileTree(data.files_list);
                }
            });
        ...
        rename: function() {
            ...
            if (self.selected_file.fileName.length > 0) {
                ...
                if (rename_to) {
                    $.ceAjax('request', fn_url('file_editor.rename_file?file=' + self.selected_file.fileName + '&file_path=' + self.selected_file.filePath + '&rename_to=' + rename_to), {cache: false, callback: function(data) {
                        initFileTree(data.files_list);
                    }});
                }
            }
        ...
        deleteFile: function() {
            ...
            if (self.selected_file.fileName.length > 0) {
                if (confirm(_.tr('text_are_you_sure_to_delete_file'))) {
                    $.ceAjax('request', fn_url('file_editor.delete_file'), {
                        data: {
                            file: self.selected_file.fileName,
                            file_path: self.selected_file.filePath
                        },
        ...
        setPerms: function() {
            ...
            if (self.selected_file.fileName.length > 0) {
                $.ceAjax('request', fn_url('file_editor.chmod'), {
                    data: {
                        file: self.selected_file.fileName,
                        file_path: self.selected_file.filePath,
                        perms: perms,
        ...
        restoreFile: function() {
            ...
            if (confirm(_.tr('text_restore_question'))) {
                $.ceAjax('request', fn_url('file_editor.restore'), {
                    data: {
                        file: self.selected_file.fileName,
                        file_path: self.selected_file.filePath
                    },
        ...
        createFile: function(filename)
        {
            ...
            $.ceAjax('request', fn_url('file_editor.create_file'), {
                data: {
                    file: file,
                    file_path: file_path
                },
        ...
        createFolder: function(folder)
        {
            ...
            $.ceAjax('request', fn_url('file_editor.create_folder'), {
                data: {
                    file: folder,
                    file_path: file_path
                },
        ...
        decompressFile: function() {
            ...
            $.ceAjax('request', fn_url('file_editor.decompress'), {
                data: {
                    file: file,
                    file_path: file_path
                },
        ...
        compressFile: function() {
            ...
            $.ceAjax('request', fn_url('file_editor.compress'), {
                data: {
                    file: file,
                    file_path: file_path
                },
